...
@Service
public class AccountUnlockOtpService {
    ...
    private static final String KEY_PREFIX = "auth:account-unlock:";
    private static final String GENERATION_COUNTER_KEY_PREFIX = "auth:account-unlock:generation-count:";
    private static final String RESEND_COUNTER_KEY_PREFIX = "auth:account-unlock:resend-count:";
    ...
    public OtpDispatchResult requestOtp(UUID userId, String originatingIpHash) {
        ...
        if (values.isEmpty()) {
            ...
            return issueOtp(userId, originatingIpHash);
        }
        ...
        redisTemplate.opsForHash().putAll(key, Map.of(
                ...
                "originatingIpHash", originatingIpHash
        ));
    ...
    public OtpVerificationResult verifyOtp(UUID userId, String providedOtp) {
        ...
        if (values.isEmpty()) {
            throw new BadRequestException("Account unlock code has expired or is invalid");
        }
        ...
        if (attempts > MAX_ATTEMPTS) {
            ...
            throw new BadRequestException("Account unlock verification attempts exceeded. Request a new code.");
        }
        ...
        if (!MessageDigest.isEqual(hashOtp(providedOtp).getBytes(StandardCharsets.UTF_8), expectedHash.getBytes(StandardCharsets.UTF_8))) {
            throw new BadRequestException("Invalid account unlock code. " + (MAX_ATTEMPTS - attempts) + " attempts remaining.");
        }

        String originatingIpHash = String.valueOf(values.get("originatingIpHash"));
        long remainingSeconds = remainingSeconds(key);
        redisTemplate.delete(key);
        return new OtpVerificationResult(true, remainingSeconds, originatingIpHash);
    }
    ...
    private OtpDispatchResult issueOtp(UUID userId, String originatingIpHash) {
        Instant now = Instant.now(clock);
    ...
    private String generateOtp(UUID userId, long createdAtEpochSecond) {
        try {
            ...
            byte[] hash = mac.doFinal((userId + ":" + createdAtEpochSecond + ":account-unlock").getBytes(StandardCharsets.UTF_8));
            int code = (ByteBuffer.wrap(hash, 0, Integer.BYTES).getInt() & Integer.MAX_VALUE) % 1_000_000;
            return String.format("%06d", code);
        } catch (Exception ex) {
            throw new IllegalStateException("Failed to generate account unlock OTP", ex);
        }
    }
    ...
    public record OtpVerificationResult(boolean verified, long expiresInSeconds, String originatingIpHash) {
    }
}
